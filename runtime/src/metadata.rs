...
decl_module! {
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		pub fn store_meta(origin, ipfs_hash: Vec<u8>, owner: T::AccountId, meta_json: Vec<u8>, price: T::Balance) -> Result {
			...
			let sender = ensure_signed(origin)?;
			Self::_check_valid_hash(&ipfs_hash)?;

			ensure!(meta_json.len() <= BYTEARRAY_LIMIT, ERR_BYTEARRAY_LIMIT);

			Self::_check_valid_meta(&meta_json)?;
...
impl<T: Trait> Module<T> {
	fn _check_valid_hash(ipfs_hash: &Vec<u8>) -> Result {
	...
	fn _check_valid_meta(meta_json: &Vec<u8>) -> Result {
